...
class V3LLM(nn.Module):
    def __init__(self, config):
        ...
        if not self.debug:
            ...
            for i in range(self.max_obj_num):
                new_tokens.append(f"<OBJ{i:03}>")
            self.objid_start_idx = self.ori_vocab_size = len(self.llama_tokenizer)
            self.llama_tokenizer.add_tokens(new_tokens, special_tokens=True)
            self.objid_end_idx = len(self.llama_tokenizer)
            print(f"llama_tokenizer:{len(self.llama_tokenizer)}")
            if self.config.qa_tokens:
                new_tokens = []
                new_tokens.append(f"<Question>")
                new_tokens.append(f"<Answer>")
                self.llama_tokenizer.add_tokens(new_tokens, special_tokens=True)
                print(f"llama_tokenizer:{len(self.llama_tokenizer)}")
    ...
    def assign_visual_hidden_states_to_llm(self, visual_hidden_states):
        if self.config.model.use_lora:
            layers = self.llama_model.model.model.layers
        else:
            layers = self.llama_model.model.layers
        for layer in layers:
            if hasattr(layer, "condition_visual_hidden_states"):
                layer.condition_visual_hidden_states(visual_hidden_states)
